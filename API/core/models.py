...
class CustomUserManager(BaseUserManager):
    ...
    def create(self, email, first_name, last_name, company, address, DOB, password=None):
        print('***********************************************************************************')
        if not email:
...
class SuscribedPlan(models.Model):
    suscribeduser = models.OneToOneField(
        Manager, on_delete=models.CASCADE)
    plan = models.ForeignKey(Plan, on_delete=models.CASCADE)
    is_active = models.BooleanField(default=False)
    suscribed_date = models.DateField(null=True)

    def __str__(self) -> str:
        return f'{self.suscribeduser} has {self.plan}'
